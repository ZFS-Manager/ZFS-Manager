...
export default function PerformanceMonitor() {
  ...
  useEffect(() => {
    // Generate mock stats for now as the backend might not have history
    const generateStats = () => {
      const now = new Date();
      const newStats: DiskStat[] = [];
      for (let i = 0; i < 20; i++) {
        newStats.push({
          timestamp: new Date(now.getTime() - (20 - i) * 5000).toLocaleTimeString(),
    ...
    <div className="glass-panel p-10">
      <div className="flex justify-between items-end mb-12">
        ...
        <div className="flex gap-12">
          ...
          <div className="text-right">
            <p className="text-[10px] font-bold text-white/20 uppercase tracking-[0.2em] mb-2 flex items-center justify-end gap-2">
              <ArrowUp size={12} className="text-blue-400" /> Write
            </p>
